...
from Model.MutualInformation import MutualInformation
...
def LDM_loss(flow, fixed_ldm, moving_ldm):
    '''
    Caculate landmark distance
    multi landmarks support
    '''
    fixed_np = fixed_ldm.cpu().detach().numpy()

    ind_x, ind_y, ind_z = fixed_np.T
    diff = flow[:, :, ind_x, ind_y, ind_z].T
    diff = diff.reshape(fixed_ldm.shape)

    flowed_pt = diff + fixed_ldm
    return torch.sqrt(torch.sum(torch.square(flowed_pt - moving_ldm), dim=1))


def MI_loss(I, J):
    MI = MutualInformation(num_bins=256, sigma=0.4, normalize=True).to(
